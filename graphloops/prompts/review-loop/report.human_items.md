...
機械の知らせ（record.process.notices。宣言に無い・rules が埋める --input の鍵、宣言の読まない段、宣言外で探した変異の実行器、結果の来ない線）: {{?record.process.notices}}
空でなければ、「ループが自分で決めたこと」の後に「機械の知らせ」の節を置き、1 件ずつ、何が起きたか・どうすれば消えるかを書く。決める事柄ではないので冒頭 3 行には混ぜない。
