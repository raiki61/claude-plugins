...
def test_cond_view_without_state_ignores_unevaluable():
    ...
    from engine.board import CondView
    CondView("c", (), {}).unevaluable("a", "測れない")   # 落ちない


def test_choice_input_errors_names_values_outside_the_declaration():
    """graph が choice と宣言した入力に、宣言の values に無い値が来たら誤りの文を返す（rules の検査に頼らない engine の口）"""
    g = {"inputs": {"gates": {"kind": "choice", "values": ["merge"]}, "cwd": {"kind": "dir"}}}
    assert commands.choice_input_errors(g, {"gates": "merge"}) == []
    errs = commands.choice_input_errors(g, {"gates": "all"})
    assert len(errs) == 1 and "--input gates='all' は知らない値" in errs[0]


def test_git_hands_the_launch_failure_reason_to_why(monkeypatch):
    """git を起こせなかった（OSError・時間切れ）回も、why を渡されていれば理由を足す（戻り値は None のまま）"""
    def boom(*a, **k):
        raise OSError("git が無い（検査用）")
    monkeypatch.setattr(util.subprocess, "run", boom)
    why = []
    assert util.git("status", why=why) is None and why == ["OSError: git が無い（検査用）"]


def test_copy_worktree_needs_a_repository_root(tmp_path, monkeypatch):
