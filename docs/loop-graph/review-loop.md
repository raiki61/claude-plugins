...
- 局所レビューはまとめ役（`review-pr`）を挟まず、レンズを 1 本ずつ名指しで起こす（名指しの正本は `graphs/review-loop.json` の `p1.local_review.skills`。ここに一覧と本数を写すと、レンズを足し引きした周に写しだけが取り残される）。`/simplify` は指摘だけ返す形で呼ぶ。**散文版（`/review-loop`）とは道具構成が割れている**——散文版はまとめ役 `review-pr` を使い、`pr-test-analyzer` は 1 度も呼ばない。散文版の道具選定はこのグラフの担当範囲の外なので揃えていない
