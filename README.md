...
`/review-loop` を使う前に、対象リポジトリで一度だけ:

```
/review-loop-init
```

これがレビュー観点の正本 `REVIEW.md` をリポジトリルートに配置し、固有欄（アーキテクチャ境界・標準機構・機械強制されている検査）をそのリポジトリを調べて埋める。**観点の正本は対象リポジトリ側に置く設計**で、プラグインが配るのは言語非依存のテンプレートだけ。観点は使う人が自分の事故から育てる資産なので（`REVIEW.md` の「この規約の育て方」）、プラグイン側に固定しない。
...
| 依存 | 用途 | 無いとき |
|---|---|---|
| `pr-review-toolkit` プラグイン | 欠陥の局所レビュー（`review-pr`） | 別途 `/plugin install`。導入後 `/reload-plugins` |
