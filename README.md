...
環境（OS・道具・権限）のせいで走れなかった検査は、合格の行と別に `# SKIP <理由>` の印つきの行で出て、末尾に「見送り N 件」の一覧が出る。見送りを失敗に数えるなら `FAIL_ON_SKIP=1 bash tests/run.sh`（既定では数えない。どの環境で数えるかは CI の定義の側で決める）。
